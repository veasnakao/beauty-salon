...
//profitAndLoss with incomeByStaff
Meteor.methods({
    profitAndLoss(fromDate, toDate){
        fromDate = moment(fromDate).startOf('days').toDate();
        toDate = moment(toDate).endOf('days').toDate();
        let profitAndLoss = Collection.JournalEntry.aggregate([
            ...
            },
            {
                $lookup: {
                    from: "incomeByStaff",
                    localField: "incomeByStaffId",
                    foreignField: "_id",
                    as: "incomeByStaffDoc"
                }
            },
            {
                $unwind: { path: '$incomeByStaffDoc', preserveNullAndEmptyArrays: true }
            },
            {
                $unwind: { path: '$incomeByStaffDoc.incomeByStaff', preserveNullAndEmptyArrays: true }
            },
            {
                $group: {
                    _id: {
                    ...
                    totalIncomeByStaff: {
                        $sum: '$incomeByStaffDoc.incomeByStaff.amount'
                    }
                }
            },
            {
            ...
            },
            {
                $project: {
                    ...
                    total: {
                        $add: ["$journalTotal", "$totalIncomeByStaff"]
                    }
                }
            },
            {
